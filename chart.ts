...
export function initCharts(
    ...
    donutCanvas: HTMLCanvasElement
): void {

    const labels: string[] = Array.from({ length: 20 }, (_, i) =>
    i === 19 ? 'now' : `-${(19 - i) * 5}s`);
    ...
    donutChart = new Chart(donutCanvas, {
        type: 'doughnut', data: {
            ...
            datasets: [{
                ...
                backgroundColor: EVENT_TYPES.map(t => EVENT_COLORS[t as EventType]),
                borderWidth:     0,
                hoverOffset:     4,
            }],
        },
...
export function updateCharts(
    ...
    if (!sparkChart || !donutChart) return;
    ...
    const counts: number[] = EVENT_TYPES.map(type =>
