...
// Have to do this to allow window.emitManual
declare global {
    interface Window {
        emitManual: () => void;
        toggleAuto: () => void;
...
const sparkCanvas = document.getElementById('sparkChart') as HTMLCanvasElement;
...
const typeSelect = document.getElementById('e-type') as HTMLSelectElement;
const pageInput = document.getElementById('e-page') as HTMLInputElement;
...
function renderMetrics(): void {
    ...
    const perMin = events.filter(e => now - e.ts < 60_000).length;
...
function renderBars(): void {
    ...
    for (const e of events) counts[e.page] = (counts[e.page] ?? 0) + 1;
...
function renderLegend(): void {
    ...
    legendEl.innerHTML = EVENT_TYPES.map(type => {
        ...
        return `<div class="legend-row">
            <div class="legend-dot" style="background:${EVENT_COLORS[type as EventType]}"></div>
...
const PAGES: string[] = ['/home', '/pricing', '/docs', '/blog', '/login', '/dashboard'];
const TYPES: EventType[] = ['page_view', 'page_view', 'page_view', 'ride_posted', 'ride_posted', 'error', 'seat_reserved', 'signup'];
 
function randomFrom<T>(arr: T[]): T | undefined {
    return arr[Math.floor(Math.random() * arr.length)]!;
}
 
window.emitManual = function (): void {
...
window.toggleAuto = function (): void {
    if (autoTimer) {
...
const seeds: Array<[EventType, string]> = [
    ['page_view', '/home'],
    ['page_view', '/pricing'],
